...
class PartnerCenter {
  ...
  async getAllCustomers(): Promise<object[]> {
    const res = await this._partnerCenterRequest('https://api.partnercenter.microsoft.com/v1/customers')
    return res.items
  }

  async getInvoices(): Promise<object[]> {
    const res = await this._partnerCenterRequest('https://api.partnercenter.microsoft.com/v1/invoices')
    return res.items
  }
  ...
  async getCustomerById(customerId: string): Promise<object> {
    const res = await this._partnerCenterRequest(`https://api.partnercenter.microsoft.com/v1/customers/${customerId}`)
    return res
  }
